...
var gridfill = {
  ...
  placeTile: function(backfill) {
    ...
    newTile.setAttribute('class', this.options.tileClass);
  ...
  resetDataIndex: function() {
    ...
    for (i in elems) {
      if((' ' + elems[i].className + ' ').indexOf(' ' + this.options.tileClass + ' ') > -1) {
        elementCount++;
      }
    }
...
gridfill.initialize({ cols: 5, tile_ratio: '4:3', selector: 'container', tileClass: 'tile', data: testData });
